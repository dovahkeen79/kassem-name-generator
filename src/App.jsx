...
function smartFilter(pool,sel,sw,ew,seen){
  ...
  const scored=f.map(n=>{
    ...
    if(vibe.length){const vm={"Dreamy & Poetic":["R","E","L"],"Bold & Fierce":["S","R"],"Gentle & Warm":["V","E"],"Intellectual & Deep":["W","N"],"Playful & Bright":["V","L","H"],"Spiritual & Sacred":["F","G"],"Confident & Strong":["S","E"],"Calm & Serene":["E","N"]};vibe.forEach(v=>{(vm[v]||[]).forEach(t=>{if(n.t.includes(t))sc+=1;});});}
    sc+=(n.u-1)*0.2;return{...n,sc};
  }).sort((a,b)=>b.sc-a.sc);
      ...
      <div style={{maxWidth:700,margin:"0 auto",padding:"clamp(20px,5vw,40px) clamp(12px,4vw,20px) 60px"}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:"clamp(4px,1.5vw,7px)",marginBottom:editF?10:0}}>
              {sibs.map((s,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:5,background:ac("0.12"),border:`1px solid ${ac("0.3")}`,borderRadius:20,padding:"clamp(3px,1vw,5px) clamp(8px,2.5vw,12px)"}}>
                  <span style={{color:T.ac,fontSize:"clamp(11px,3vw,13px)"}}>Sibling {i+1}: {s}</span>
                  {editF&&<span onClick={()=>setSibs(p=>p.filter((_,j)=>j!==i))} style={{color:"rgba(255,100,100,0.7)",cursor:"pointer",fontSize:"clamp(13px,3.5vw,16px)",lineHeight:1}}>×</span>}
                </div>
              ))}
        ...
        {showF&&favs.length>0&&(
          <div style={{background:ac("0.05"),border:`1px solid ${ac("0.25")}`,borderRadius:"clamp(12px,3vw,16px)",padding:"clamp(14px,4vw,20px)",marginBottom:"clamp(14px,4vw,20px)",animation:"fadeUp 0.35s ease"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"clamp(10px,3vw,14px)"}}>
              <span style={{color:T.ac,fontSize:"clamp(12px,3.5vw,14px)",fontWeight:"bold"}}>❤️ FAVOURITES ({favs.length})</span>
              <span onClick={()=>setFavs([])} style={{color:"rgba(255,100,100,0.5)",fontSize:"clamp(11px,3vw,13px)",cursor:"pointer"}}>Clear all</span>
            </div>
            {favs.map((item,i)=>(
              <div key={i} className="nc" style={{background:ac("0.07"),border:`1px solid ${ac("0.3")}`}}>
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3,flexWrap:"wrap"}}>
                    <span style={{color:T.ac,fontSize:"clamp(16px,5vw,20px)",fontWeight:"bold"}}>{item.n}</span>
                    <span style={{background:ac("0.12"),color:ac("0.75"),borderRadius:6,padding:"2px 8px",fontSize:"clamp(9px,2.5vw,11px)"}}>{UL[item.u]}</span>
                  </div>
                  <div style={{color:ac("0.65"),fontSize:"clamp(10px,2.8vw,12px)",marginBottom:2}}>{item.o}</div>
                  <div style={{color:"rgba(255,255,255,0.45)",fontSize:"clamp(10px,2.8vw,12px)",marginBottom:2}}>{item.m}</div>
                  <div style={{color:"rgba(255,255,255,0.22)",fontSize:"clamp(9px,2.5vw,11px)",fontStyle:"italic"}}>{ff(item.n)}</div>
                </div>
                <button onClick={()=>togF(item)} style={{background:"transparent",border:"none",cursor:"pointer",fontSize:"clamp(18px,5vw,22px)",flexShrink:0}}>💔</button>
              </div>
            ))}
          </div>
        )}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"clamp(8px,2.5vw,12px)",marginBottom:"clamp(8px,2.5vw,12px)"}}>
          {[["🔡 Starts With",sw,setSw,"e.g. Sa"],["🔤 Ends With",ew,setEw,"e.g. ina"]].map(([lbl,val,set,ph],i)=>(
            <div key={i} style={{background:ac("0.04"),border:`1px solid ${ac("0.25")}`,borderRadius:"clamp(10px,3vw,14px)",padding:"clamp(10px,3vw,14px)"}}>
              <div style={{color:T.ac,fontSize:"clamp(10px,2.8vw,12px)",fontWeight:"bold",marginBottom:7}}>{lbl}</div>
        ...
        {CATS.map(cat=>{
          ...
          return(
            <div key={cat.key} className="cat" style={{background:s.length?ac("0.04"):"rgba(255,255,255,0.02)",border:`1px solid ${s.length?ac("0.3"):"rgba(255,255,255,0.06)"}`}}>
              <div onClick={()=>setColl(p=>({...p,[cat.key]:!p[cat.key]}))} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"clamp(10px,3vw,13px) clamp(12px,4vw,16px)",cursor:"pointer",background:s.length?ac("0.05"):"transparent"}}>
                <div style={{display:"flex",alignItems:"center",gap:9}}>
                  <span style={{fontSize:"clamp(13px,4vw,16px)"}}>{cat.icon}</span>
                  <span style={{color:s.length?T.ac:"rgba(255,255,255,0.65)",fontSize:"clamp(11px,3.2vw,13px)",fontWeight:"bold"}}>{cat.label}</span>
                  {s.length>0&&<span style={{background:T.badge,color:T.bt,borderRadius:10,padding:"1px 7px",fontSize:"clamp(9px,2.5vw,11px)",fontWeight:"bold"}}>{s.length}</span>}
                </div>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  {s.length>0&&<span onClick={e=>{e.stopPropagation();setSel(p=>({...p,[cat.key]:[]}))} } style={{color:"rgba(255,100,100,0.6)",fontSize:"clamp(10px,2.8vw,12px)",cursor:"pointer"}}>Clear</span>}
                  <span style={{color:"rgba(255,255,255,0.25)",fontSize:"clamp(10px,2.8vw,12px)"}}>{open?"▾":"▸"}</span>
                </div>
              </div>
              {open&&(
                <div style={{padding:`2px clamp(12px,4vw,16px) clamp(10px,3vw,14px)`}}>
                  {s.length>0&&<div style={{fontSize:"clamp(10px,2.8vw,12px)",color:ac("0.55"),fontStyle:"italic",marginBottom:9}}>✓ {s.join(" · ")}</div>}
                  <div style={{display:"flex",flexWrap:"wrap",gap:"clamp(5px,1.8vw,8px)"}}>
                    {cat.opts.map(o=>{
                      const on=s.includes(o);
                      return<button key={o} className="pill" onClick={()=>tog(cat.key,o)} style={{borderColor:on?T.ac:"rgba(255,255,255,0.1)",background:on?ac("0.16"):"rgba(255,255,255,0.02)",color:on?T.ac:"rgba(255,255,255,0.45)"}}>
                        {on&&<span style={{fontSize:"clamp(8px,2.2vw,10px)",fontWeight:"bold"}}>✓</span>}{o}
                      </button>;
                    })}
                  </div>
                </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:ac("0.06"),border:`1px solid ${ac("0.15")}`,borderRadius:"clamp(8px,2.5vw,12px)",padding:"clamp(8px,2.5vw,12px) clamp(12px,4vw,16px)",marginBottom:"clamp(10px,3vw,14px)"}}>
          <span style={{fontSize:"clamp(11px,3vw,13px)",color:"rgba(255,255,255,0.4)"}}>
            {tot>0?<><span style={{color:T.ac,fontWeight:"bold"}}>{tot} prefs</span> · {Object.values(sel).filter(v=>v.length>0).length} categories</>:<span style={{color:"rgba(255,255,255,0.3)"}}>No filters — showing diverse results</span>}
          </span>
          <span onClick={resetAll} style={{color:"rgba(255,100,100,0.5)",fontSize:"clamp(11px,3vw,13px)",cursor:"pointer"}}>Reset all</span>
        </div>
        <button onClick={generate} style={{width:"100%",padding:"clamp(13px,4vw,16px)",borderRadius:"clamp(10px,3vw,14px)",border:`1px solid ${T.ac}`,background:`linear-gradient(135deg,${ac("0.22")},${ac("0.1")})`,color:T.ac,fontSize:"clamp(12px,3.5vw,15px)",fontWeight:"bold",letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:"clamp(16px,5vw,22px)",transition:"all 0.3s"}}>
          ✨ Generate {T.lb} Names
        </button>
        {gen&&(
          <div style={{animation:"fadeUp 0.4s ease"}}>
            {noRes?(
              <div style={{background:ac("0.05"),border:`1px solid ${ac("0.2")}`,borderRadius:14,padding:"20px",textAlign:"center",marginBottom:16}}>
                <div style={{fontSize:28,marginBottom:8}}>🔍</div>
                <div style={{color:T.ac,fontSize:14,marginBottom:6}}>No names match your filters</div>
                <div style={{color:"rgba(255,255,255,0.4)",fontSize:12}}>Try removing some filters</div>
              </div>
            ):(
              <>
                <div style={{textAlign:"center",marginBottom:"clamp(10px,3vw,14px)"}}><span style={{color:T.ac,fontSize:"clamp(10px,2.8vw,12px)",letterSpacing:"0.12em",textTransform:"uppercase"}}>❆ {T.lb} Names — Tap ❤️ to save ❆</span></div>
                {res.map((item,i)=>(
                  <div key={i} className="nc" style={{background:isF(item)?ac("0.07"):"rgba(255,255,255,0.025)",border:`1px solid ${isF(item)?ac("0.45"):"rgba(255,255,255,0.06)"}`,animation:`fadeUp 0.3s ease ${i*0.04}s both`}}>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",flexWrap:"wrap",gap:8,marginBottom:4}}>
                        <span style={{color:T.ac,fontSize:"clamp(17px,5vw,21px)",fontWeight:"bold"}}>{item.n}</span>
                        <span style={{background:ac("0.1"),color:ac("0.75"),borderRadius:6,padding:"2px 8px",fontSize:"clamp(9px,2.5vw,10px)"}}>{UL[item.u]}</span>
                      </div>
                      <div style={{color:ac("0.65"),fontSize:"clamp(10px,2.8vw,12px)",marginBottom:2}}>{item.o}</div>
                      <div style={{color:"rgba(255,255,255,0.45)",fontSize:"clamp(10px,2.8vw,12px)",marginBottom:2}}>{item.m}</div>
                      <div style={{color:"rgba(255,255,255,0.22)",fontSize:"clamp(9px,2.5vw,11px)",fontStyle:"italic"}}>{ff(item.n)}</div>
                    </div>
                    <button onClick={()=>togF(item)} style={{background:"transparent",border:"none",cursor:"pointer",fontSize:"clamp(18px,5vw,22px)",flexShrink:0}}>{isF(item)?"❤️":"🤍"}</button>
                  </div>
                ))}
                <div style={{display:"flex",gap:"clamp(6px,2vw,10px)",marginTop:4}}>
                  ...
                  {favs.length>0&&<button onClick={()=>{setShowF(true);window.scrollTo({top:0,behavior:"smooth"})}} style={{flexShrink:0,padding:"clamp(10px,3vw,13px) clamp(14px,4vw,18px)",borderRadius:"clamp(8px,2.5vw,12px)",border:`1px solid ${ac("0.4")}`,background:ac("0.1"),color:T.ac,fontSize:"clamp(11px,3vw,13px)",cursor:"pointer"}}>❤️ {favs.length}</button>}
                </div>
              </>
            )}
          </div>
        ...
        <p style={{textAlign:"center",color:"rgba(255,255,255,0.1)",fontSize:"clamp(9px,2.5vw,11px)",marginTop:"clamp(20px,5vw,32px)",letterSpacing:"0.05em"}}>
          {GIRLS.length+BOYS.length}+ names · No API needed · Kassem Family · London
        </p>
      </div>
    </div>
  );
